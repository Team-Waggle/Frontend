...
const CurrentPositionField = forwardRef<HTMLDivElement, Props>(
  ({ value = [], onChange, isValidated }, ref) => {
    ...
    return (
      <div
        ref={ref}
        className="flex flex-col gap-[2.4rem] border-t-2 border-solid border-black-40 pt-[2rem]"
      >
            ...
            <div className="flex flex-col gap-[0.8rem]">
              {index === 0 && (
                <FormLabel
                  title="현재 참여중인 인원"
                  isRequired
                  requiredMessage={isValidated && !block.position.trim()}
                />
              )}
              <BaseSelect
                items={positions}
