...
const Drawer = ({ children }: DrawerProps) => {
  const { isOpen: filterIsOpen, close: filterIsClose } = useFilterStore();
  const { isOpen: IpnIsOpen, close: IpnIsClose } = useIpnStore();

  return (
    <AnimatePresence>
      {/* isOpen이 true일 때만 컴포넌트들을 렌더링합니다. */}
      {(filterIsOpen || IpnIsOpen) && (
        <>
          <motion.div
            className="fixed bottom-0 left-0 right-0 top-[7rem] z-40 bg-black/40 md:hidden"
            onClick={filterIsClose || IpnIsClose}
