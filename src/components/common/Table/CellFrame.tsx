...
const bodyVariant: Record<ColumnVariant, string> = {
  ...
  skill: 'flex w-[14.4rem] h-[6rem] justify-start items-center flex-shrink-0 gap-[1rem]',
  title:
...
export function ThFrame({
  ...
  return (
    <div
      className={`bg-black-10 text-caption-12_M500 text-black-70 border border-solid border-black-70 border-x-0 border-t-0 ${headVariant[variant]} ${className ?? ''}`}
    >
      {children}
    </div>
  );
